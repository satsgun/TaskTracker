...
class TestUpdateTask:
    def test_update_description_only_returns_204(self):
    ...
    def test_update_description_only_changes_description_and_preserves_status(self):
        task = _create_task(description="Buy milk")
        client.patch(f"/tasks/{task['id']}/", json={"status": "Complete"})

        client.patch(f"/tasks/{task['id']}/", json={"description": "Buy oat milk"})

        updated = _get(task["id"])
        assert updated["description"] == "Buy oat milk"
        assert updated["status"] == "Complete"

    def test_update_status_only_preserves_description(self):
    ...
    def test_update_description_and_status_together(self):
    ...
    def test_update_with_whitespace_only_description_returns_422(self):
    ...
    def test_update_with_invalid_status_returns_422(self):
    ...
    def test_update_with_empty_body_returns_422(self):
        task = _create_task(description="File taxes")
